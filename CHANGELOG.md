...
- `translate-frontmatter-fields` input (default `false`): translates the `title`/`description`/`sidebar_label`/`label` YAML/TOML frontmatter values - the ones that show up in page navigation and browser tabs - while leaving `slug`, `sidebar_position`, `tags`, dates, booleans, numbers, and any other key byte-for-byte untouched. Fixed key allowlist plus a scalar-value shape check (`FrontmatterFieldExtractor`), not a YAML/TOML parser; a value that doesn't cleanly match is left alone rather than guessed at, same philosophy as `translate-mermaid-diagrams`.
