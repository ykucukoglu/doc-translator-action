...
- Fork `pull_request_target` safety: automatically forces a dry run when triggered by that event/fork-PR combination, regardless of `pr-mode`, unless `allow-fork-pull-request-target: true` is set.
